...
@testset "convert(::Type{MaximumLikelihoodAbstractExtremeValueModel}, fm::PseudoMaximaEVA, iter::Int)" begin
    ...
    res = convert(MaximumLikelihoodAbstractExtremeValueModel, fm, 2)
...
@testset "quantile(::PseudoMaximaEVA, order)" begin
    ...
    @testset "stationary model" begin
        ...
        @test all(quantile(fm, .9) .== quantile.(pd, .9))
